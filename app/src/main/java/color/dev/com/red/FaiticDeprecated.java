...
public class FaiticDeprecated implements Serializable {
    ...
    public FaiticDeprecated(boolean verbose) {
    ...
    public static String requestDocument(String strurl, String post) throws Exception {
        ...
        List<HttpCookie> cookiesAssoc = cookieManager.getCookieStore().get(ADAPT(lastRequestedURL, url).toURI());
        ...
        cookieManager.setCookiePolicy(CookiePolicy.ACCEPT_ALL); //BUG
            ...
            if (headerName.toLowerCase().equals("set-cookie")) {
                ...
                for (String cookieToSet : cookiesToSet.split(";")) {
                    ...
                    if (cookieParameters[0].contains("path") || cookieParameters[0].contains("expire")) {
                        Log.v("GALLETA NO CARGADA", cookieParameters[0]);
                    } else {

                        HttpCookie galleta = new HttpCookie(cookieParameters[0].trim(), cookieParameters[1].trim());
                        galleta.setPath("/");

                        cookieManager.getCookieStore().add(ADAPT(lastRequestedURL, url).toURI(), galleta);

                        logger.log(Logger.INFO, " + Adding cookie \"" + cookieToSet + "\" to uri \"" + ADAPT(lastRequestedURL, url).toURI().toString() + "\".");
        ...
        if (status == HttpURLConnection.HTTP_MOVED_TEMP
            ...
            String[] textodividido = output.toString().split("\n");

            for (String linea : textodividido)
                Log.v("Salida", linea);
        ...
        for (int i = 1; (headerName = connection.getHeaderFieldKey(i)) != null; i++) {
            ...
            if (headerName.toLowerCase().equals("set-cookie")) {
                ...
                for (String cookieToSet : cookiesToSet.split(";")) {
                    ...
                    cookieManager.getCookieStore().add(ADAPT(lastRequestedURL, url).toURI(), new HttpCookie(cookieParameters[0].trim(), cookieParameters[1].trim()));
    ...
    static URL ADAPT(String lastRequestedURL, URL u2) {


        return u2;
		/*
		String u = u2.toString();
		String r = u.replace(" ","%20").replace("https:","http:");


		if(!("@@@@"+u).contains("@@@@http")){
			if((""+u.charAt(0)+"").contains("/")) {
				r = "http://cursos.faitic.uvigo.es" + u;
			}else {
				r = "http://cursos.faitic.uvigo.es/" + u;
			}

			Log.v("he modificado la url",r);
		}
